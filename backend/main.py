...
def parse_expense_command(text: str):
    pattern = r'\[EXPENSE:([^\|]+)\|([^\|]+)\|([^\]]+)\]'
    match = re.search(pattern, text)
    if match:
...
def parse_event_command(text: str):
    pattern = r'\[EVENT:([^\|]+)\|([^\|]+)\|([^\]]+)\]'
    match = re.search(pattern, text)
    if match:
...
async def chat_stream(req: ChatStreamRequest):
    ...
    async def generate():
        ...
        if full_reply:
            # 解析并执行指令
            reminder = parse_reminder_command(full_reply)
            if reminder:
                try:
                    supabase.table("reminders").insert({"content": reminder["content"], "remind_at": reminder["time"], "is_done": False}).execute()
                    print(f"✅ 创建提醒: {reminder}")
                except Exception as e:
                    print(f"❌ 创建提醒失败: {e}")
            
            expense = parse_expense_command(full_reply)
            if expense:
                try:
                    supabase.table("expenses").insert({"amount": float(expense["amount"]), "category": expense["category"], "note": expense["note"], "expense_at": datetime.utcnow().isoformat()}).execute()
                    print(f"✅ 记账: {expense}")
                except Exception as e:
                    print(f"❌ 记账失败: {e}")
            
            event = parse_event_command(full_reply)
            if event:
                try:
                    supabase.table("calendar_events").insert({"title": event["title"], "description": event["description"], "event_at": event["time"]}).execute()
                    print(f"✅ 创建日程: {event}")
                except Exception as e:
                    print(f"❌ 创建日程失败: {e}")
            
            clean_reply = remove_commands(full_reply)
            
            # 存储AI回复
            try:
                supabase.table("chat_messages").insert({"chat_id": req.chat_id, "role_id": req.role_id, "sender": "assistant", "content": clean_reply, "metadata": {"role_name": role_name}}).execute()
                print(f"✅ AI回复已存储")
            except Exception as e:
                print(f"❌ 存储AI回复失败: {e}")
