"""
AI Assistant Backend Service - 适配新表结构
- 聊天记录 chat_messages
- GPS轨迹 gps_history  
- 用户画像 user_persona
- AI行为设置 ai_behavior_settings
- 主动消息 proactive_messages
- 闹钟 reminders
- 记账 expenses
- 日历 calendar_events
- 通知采集 notifications
...
class MemoryCreate(BaseModel):
    title: Optional[str] = None
    content: str
    category: Optional[str] = None  # note, event, summary, important
...
def get_all_roles() -> List[dict]:
    ...
    if not supabase:
        return []
    try:
        result = supabase.table("user_sync").select("roles").eq("user_id", "default_user").limit(1).execute()
...
def build_role_system_prompt(role: dict) -> str:
    """从角色设置构建 system prompt，完全由用户填写决定，不注入硬编码描述"""
    if not role:
        return ""
    parts = []
    if role.get("persona"):
...
def get_ai_behavior_settings() -> dict:
    """读取 ai_behavior_settings 表"""
    if not supabase:
        return {}
    try:
        result = supabase.table("ai_behavior_settings").select("setting_name, setting_value").execute()
        return {row["setting_name"]: row["setting_value"] for row in result.data} if result.data else {}
    except Exception as e:
        print(f"⚠️ 读取 ai_behavior_settings 失败: {e}")
        return {}
...
def get_user_persona_summary() -> str:
    ...
    try:
        ...
        if not result.data:
            return ""
        return "\n".join([f"[{r['trait_category']}] {r['trait_detail']} (确定度{r.get('confidence_score',0):.1f})" for r in result.data])
    except Exception as e:
        print(f"⚠️ 读取 user_persona 失败: {e}")
        return ""
...
async def lifespan(app: FastAPI):
    ...
    scheduler.add_job(proactive_thinking, 'interval', minutes=5, id='proactive_thinker')
    scheduler.add_job(async_update_persona, 'interval', hours=24, id='persona_updater')
    scheduler.start()
